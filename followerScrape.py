...
def Scrape(id):
        ...
        while len(que) > 0:
                ...
                G_dict[ID]=[]
                que.pop(0)
                response2=client.get_users_followers(ID, expansions=None, max_results=1000, user_fields=['verified','public_metrics'])
                ...
                if response2.data is not None:
                        for user in response2.data:
                                ...
                                if user.id in G_dict:
                                    G_dict[user.id].append(ID)
                                else:
                                    G_dict[user.id]=[ID] 
